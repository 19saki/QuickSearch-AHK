...
; 读取当前自启状态
Global g_AutoStart := RegRead(REG_KEY, REG_NAME, "") != "" ? true : false

A_TrayMenu.Delete()  ; 清空默认菜单
A_TrayMenu.Add("打开搜索菜单 (Ctrl+Alt+S)", (*) => g_SearchManager.Show())
A_TrayMenu.Add()  ; 分隔线
...
A_TrayMenu.Default := "打开搜索菜单 (Ctrl+Alt+S)"

ToggleAutoStart(*) {
    Global g_AutoStart, REG_KEY, REG_NAME
    if (g_AutoStart) {
        RegDelete(REG_KEY, REG_NAME)
        A_TrayMenu.Uncheck("开机自启动")
        g_AutoStart := false
        QuickTip("已取消开机自启")
    } else {
        RegWrite('"' . A_ScriptFullPath . '"', "REG_SZ", REG_KEY, REG_NAME)
        A_TrayMenu.Check("开机自启动")
        g_AutoStart := true
        QuickTip("已设置开机自启")
    }
